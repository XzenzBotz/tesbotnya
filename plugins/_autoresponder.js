...
let loveyou = lovou[Math.floor(Math.random() * lovou.length)]
    ...
    let hellozanz = fs.readFileSync('./mp3/PTT-20211218-WA0243.opus') 
    conn.sendMessage(m.chat, {
        react: {
          text: `${loveyou}`,
          key: m.key,
   ...
   setTimeout(() => {
        conn.sendFile(m.chat, hellozanz, '', '', m, true)
    }, 1000)
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   /* let hiai = /(hi|hii|hiii|hi zanz|hii zanz|hiii zanz|hy|halo|hallo|helo|hello|hy zanz|halo zanz|hallo zanz|helo zanz|hello zanz)/i
...
let hizanz = hiain[Math.floor(Math.random() * hiain.length)]
    if (isAih && !m.fromMe) {
    let hellozanz = fs.readFileSync('./mp3/WhatsApp-Ptt-2021-07-14-at-18.12.33.opus') 
    conn.sendMessage(m.chat, {
        react: {
          text: `${hizanz}`,
          key: m.key,
